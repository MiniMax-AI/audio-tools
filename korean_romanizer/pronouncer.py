...
class Pronouncer(object):
    ...
    def final_substitute(self):
        for idx, syllable in enumerate(self._syllables):
            ...
            try:
                final_is_before_C = syllable.final and next_syllable and next_syllable.initial not in (
                    None, NULL_CONSONANT)
            except AttributeError:
                final_is_before_C = False

            try:
                final_is_before_V = syllable.final and next_syllable and next_syllable.initial in (
                    None, NULL_CONSONANT)
            except AttributeError:
            ...
            # 可能出现不合规范的情况（例如잃作为句尾，句子不完整，double consonant final出现在句尾），暂时不处理
